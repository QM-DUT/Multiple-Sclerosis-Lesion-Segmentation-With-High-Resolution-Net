...
if __name__=="__main__":
    ...
    # 遍历根目录
    index = 0
    for root, dirs, files in os.walk("/home/invisible/Desktop/DIP_Project/Pre-processed_training_dataset/"):
        if dirs == []:
            # 对每一个文件夹
            print(root)
            ...
            for d in range(0, depth - 64 + 16, 16):
                for w in range(0, width - 64 + 16, 16):
                    for h in range(0, height -64+ 16, 16):
                        ...
                        if np.sum(patch_mask) > voxel and os.path.split(root)[-1] != "01016SACH" and \
                                os.path.split(root)[-1] != "08031SEVE":
